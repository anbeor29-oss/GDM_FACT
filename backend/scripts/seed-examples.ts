/**
 * seed-examples — carga datos de ejemplo en UNA empresa para demostrar el
 * sistema completo (POS, facturación, inventario). Idempotente por SKU/RFC:
 * si ya existe no lo duplica.
 *
 * Uso:
 *   npm run seed:examples -- EKU9003173C9
 *
 * Inserta ~16 productos (con precio de menudeo, mayoreo y stock) y ~6 clientes.
 */
import { Pool } from 'pg';

const rfc = process.argv.slice(2).find((a) => !a.startsWith('--'))?.toUpperCase();
if (!rfc) {
...
// [sku, nombre, claveSat, unidad(clave/nombre), menudeo, mayoreo, stock, preset]
const PRODUCTS: [string, string, string, string, string, number, number, number, string][] = [
...
// [rfc, razón social, régimen, uso, cp, email]
...
async function main() {
  ...
  try {
    const compR = await c.query<{ id: string; business_name: string }>(
    ...
    for (const [sku, name, clave, unit, unitName, retail, wholesale, stock, preset] of PRODUCTS) {
      const dup = await c.query(`SELECT 1 FROM products WHERE company_id=$1 AND sku=$2 AND deleted_at IS NULL`, [companyId, sku]);
    ...
    for (const [crfc, bn, regime, uso, cp, email] of CUSTOMERS) {
      const dup = await c.query(`SELECT 1 FROM customers WHERE company_id=$1 AND rfc=$2 AND deleted_at IS NULL`, [companyId, crfc]);
      if (dup.rowCount) continue;
      await c.query(
    ...
    console.log(`✅ Listo: ${pAdded} productos y ${cAdded} clientes nuevos (los existentes se omiten).`);
  } finally {
    c.release();
    await pool.end();
  }
